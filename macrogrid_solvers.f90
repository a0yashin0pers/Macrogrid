module macrogrid_solvers
   ...
   public :: simple_iteration_parallel
...
contains
   ...
   subroutine simple_iteration_parallel(subgrid_solver, macrogrid,   &
      macrogrid_size_x, macrogrid_size_y,                   &
      subgrid_size, omega,                                  &
      eps_subgrid, eps_interface,                           &
      max_iter_subgrid, max_iter_interface,                 &
      ...
      do iter = 1, max_iter_interface
         ...
         !$OMP PARALLEL DO PRIVATE(iX, iY, subgrid_error) COLLAPSE(2) SCHEDULE(static) &
         !$OMP DEFAULT(NONE) SHARED(macrogrid, macrogrid_size_x, macrogrid_size_y, subgrid_size, omega, eps_subgrid, max_iter_subgrid, dx, dy)
         do iX = 1, macrogrid_size_x
            do iY = 1, macrogrid_size_y
               call subgrid_solver(macrogrid(iX,iY,:,:), subgrid_size, &
                  omega, eps_subgrid, max_iter_subgrid, subgrid_error, dx, dy)
         ...
         do iX = 1, macrogrid_size_x - 1
            do iY = 1, macrogrid_size_y
               do k = 2, subgrid_size - 1
                  old_value = macrogrid(iX, iY, subgrid_size, k)
                  new_val = (-macrogrid(iX,iY, subgrid_size-2, k) + 4.0d0*macrogrid(iX+1, iY, 2, k ) + &
                      4.0d0*macrogrid(iX, iY, subgrid_size-1, k) - macrogrid(iX+1, iY, 3, k)) / 6.d0
                  macrogrid(iX,   iY, subgrid_size, k) = new_val
                  macrogrid(iX+1, iY, 1,            k) = new_val
                  interface_error = interface_error + dabs(new_val - old_value)
               end do
         ...
         if (interface_error < eps_interface) exit
      end do
   ...
   end subroutine simple_iteration_parallel

   subroutine Cv_parallel(subgrid_solver, macrogrid, v, b, dx, dy, macrogrid_size_x, macrogrid_size_y, subgrid_size, interface_size, omega, eps_subgrid, max_iter_subgrid, Cv_vec)
      implicit none
      external :: subgrid_solver
      integer, intent(in) :: macrogrid_size_x, macrogrid_size_y, subgrid_size, interface_size, max_iter_subgrid
      real*8,  intent(in) :: v(interface_size), b(interface_size), dx, dy, omega, eps_subgrid
   ...
   end subroutine Cv_parallel

   subroutine conjugate_residuals_parallel(subgrid_solver, macrogrid,&
      macrogrid_size_x, macrogrid_size_y,                   &
      subgrid_size, omega,                                  &
      eps_subgrid, eps_interface,                           &
      max_iter_subgrid, max_iter_interface,                 &
      ...
      call Cv_parallel(subgrid_solver, macrogrid, u_old, b, dx, dy, macrogrid_size_x, macrogrid_size_y, subgrid_size, interface_size, omega, eps_subgrid, max_iter_subgrid, Ar_old)
      r_new = - b - Ar_old

      p_new = r_new

      call Cv_parallel(subgrid_solver, macrogrid, r_new, b, dx, dy, macrogrid_size_x, macrogrid_size_y, subgrid_size, interface_size, omega, eps_subgrid, max_iter_subgrid, Ar_new)
      call dot_product(Ar_new, r_new, interface_size, temp0)
      call dot_product(Ar_new, Ar_new, interface_size, temp1)
      alpha = temp0 / temp1
      ...
      call Cv_parallel(subgrid_solver, macrogrid, r_old, b, dx, dy, macrogrid_size_x, macrogrid_size_y, subgrid_size, interface_size, omega, eps_subgrid, max_iter_subgrid, Ar_old)
      call dot_product(Ar_old, r_old, interface_size, temp0)
      call dot_product(Ar_new, r_new, interface_size, temp1)
      beta = temp0 / temp1
   ...
   end subroutine conjugate_residuals_parallel
